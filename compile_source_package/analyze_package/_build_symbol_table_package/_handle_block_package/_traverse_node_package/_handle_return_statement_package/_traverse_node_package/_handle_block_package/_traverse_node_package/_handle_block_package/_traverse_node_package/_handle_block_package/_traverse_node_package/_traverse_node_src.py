...
def _traverse_node(node: AST, symbol_table: SymbolTable) -> None:
    """
    Central dispatcher for AST traversal.
    Routes nodes to appropriate handlers via dispatch table.
    """
    node_type = node.get("type", "")
    handler: Optional[HandlerFunc] = HANDLERS.get(node_type)
...
def _record_unknown_type_error(node: AST, symbol_table: SymbolTable) -> None:
    """Record error for unknown node type."""
...
# Dispatch table: maps node types to handler functions
