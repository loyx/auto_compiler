...
def _handle_var_decl(node: AST, symbol_table: SymbolTable) -> None:
    ...
    从 node 中提取变量名、数据类型、位置信息并注册到 symbol_table["variables"]。
    如果变量已声明，记录重复声明错误到 symbol_table["errors"]。
    """
    # 1. 从 node 中提取信息
    ...
    # 2. 确保 symbol_table["variables"] 存在
    if "variables" not in symbol_table:
        symbol_table["variables"] = {}
    
    # 3. 确保 symbol_table["errors"] 存在
