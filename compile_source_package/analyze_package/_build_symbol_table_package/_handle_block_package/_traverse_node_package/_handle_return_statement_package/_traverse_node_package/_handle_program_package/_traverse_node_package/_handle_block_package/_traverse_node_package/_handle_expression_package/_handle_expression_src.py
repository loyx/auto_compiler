...
def _handle_expression(node: AST, symbol_table: SymbolTable) -> None:
    """
    处理 'expression' 类型节点。遍历表达式子节点进行语义检查。
    
    处理逻辑：
    1. 从 node 获取 "children" 字段
    2. 遍历每个子节点（可能是 identifier、literal、或其他 expression）
    3. 对每个子节点调用 _traverse_node 进行递归处理
    4. （可选）根据表达式的运算符进行类型检查
    """
