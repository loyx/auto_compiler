...
# _traverse_node is the parent dispatcher function
...
def _handle_program(node: AST, symbol_table: SymbolTable) -> None:
    ...
    program 是 AST 的根节点，包含整个程序的所有顶级声明。
    遍历 program 节点的 children，对每个子节点递归调用 _traverse_node。
    
    Args:
        node: program 类型的 AST 节点
        symbol_table: 符号表，用于存储函数/变量信息和错误
    
    副作用:
        通过子节点处理间接修改 symbol_table
    """
